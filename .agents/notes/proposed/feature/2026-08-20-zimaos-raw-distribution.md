...
The systemd service sets `DSH_HOME=/var/lib/casaos/deepseek_harness`, reads the optional `/var/lib/casaos/deepseek_harness/.env`, and restarts the process after failure. The launcher creates the writable state directory, then executes the staged CLI as `dsh web --patch /usr/lib/deepseek-harness/zimaos.patch.yml --port 3080 --no-open`. The source-controlled overlay sets only the `webserver` row to `host: 0.0.0.0`; the public CLI continues to reject `--host 0.0.0.0` so other deployments cannot expose remote code execution accidentally. The service does not register a CasaOS Gateway route or depend on the CasaOS message bus.
