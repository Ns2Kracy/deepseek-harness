...
`dsh web` 默认绑定 `127.0.0.1`。web 命令拒绝通配主机名标志（`--host 0.0.0.0` 属于用法错误），使标志族保持为一份规模小、经过审慎限定的约定；显式的全接口姿态是配置层：把 webserver 行的 `host` 替换为 `0.0.0.0`（两个取值的 schema），端口仍跟随 `--port`。全接口姿态仍然输出本机环回 URL，并在可用时输出第一个外部 IPv4 URL。
...
`dsh web` 的本地启动仍可通过 `http://127.0.0.1:3080` 访问；其他机器上的浏览器通过绑定全部接口的配置层选择加入，并用 `--trusted-host` 声明其访问主机名。[Docker note](../process/2026-08-17-docker-deployment.md) 为容器随镜像发布了这样的 overlay。CLI 尚未开放自定义接口地址或 IPv6 模式，而以编程方式使用承载层的消费方仍保留这种灵活性。服务器测试将环回模式和全接口模式向 Node 监听边界的传递固定为约定，Web 冒烟测试继续覆盖默认 CLI 路径。
