...
describe('ZimaOS RAW source layout', () => {
  ...
  it('launches the bundled Web profile with only the ZimaOS overlay exposing it', async () => {
    ...
    expect(launcher).toContain('exec /usr/lib/deepseek-harness/node/bin/node')
    expect(launcher).toContain('/usr/lib/deepseek-harness/app/node_modules/@deepseek-ai/dsh/lib/bin.js')
    expect(launcher).toContain('web --patch /usr/lib/deepseek-harness/zimaos.patch.yml')
    expect(launcher).toContain('--port 3080 --no-open')
