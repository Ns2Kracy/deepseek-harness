...
function derivedScope(
  ...
  spec: {
    namespace: string
    decode?: (section: unknown) => UiTestSettings | undefined
  } = { namespace: 'ui-test' },
) {
...
function trackValues(
  scope: SettingsScope<UiTestSettings>,
): Array<UiTestSettings | undefined> {
...
describe('SettingsScopeController', () => {
  ...
  it('treats a schema envelope it cannot rehydrate as vouching for no section', async () => {
    ...
    const describeCall = vi
      ...
      .mockResolvedValueOnce(
        ok({ writable: true, hasDocument: true, namespaces: [broken] }),
      )
    const { mirror, scope } = derivedScope({ describe: describeCall })
    await mirror.load()
    expect(scope.getSnapshot()).toMatchObject({
  ...
  it('applies a custom decode override in place of the wire schema', async () => {
    ...
    const { mirror, scope } = derivedScope(
      { describe: describeCall },
      {
        namespace: 'ui-test',
        decode: section =>
          (section as UiTestSettings).preference === 'dark'
  ...
  it('serializes rapid set writes, carries revisions, and publishes only the latest settlement', async () => {
    ...
    const describeCall = vi
      .fn()
      .mockResolvedValue(described({ preference: 'system' }, 4))
    const mutate = vi
      .fn()
  ...
  it('folds the latest write answer into the mirror so a sibling scope sees it', async () => {
    const describeCall = vi
      .fn()
      .mockResolvedValueOnce(described({ preference: 'system' }, 4))
    const mutate = vi
      .fn()
      .mockResolvedValueOnce(ok(view({ preference: 'dark' }, 5)))
    ...
    expect(sibling.getSnapshot()).toMatchObject({
      value: { preference: 'dark' },
      revision: 5,
    })
  })
  ...
  it('keeps the write queue usable when a write publication listener throws', async () => {
    ...
    await expect(scope.set('preference', 'dark')).rejects.toThrow(
      'write subscriber failed',
    )
  ...
  it('ignores a mirror notification already queued when disposal starts', async () => {
    ...
    snapshot = {
      ...
      view: {
        ...snapshot.view,
        namespaces: [view({ preference: 'light' }, 2)],
      },
    }
    notify()
  ...
  it('carries the composition base and the user layer into the snapshot', async () => {
    ...
    const describeCall = vi
      .fn()
      .mockResolvedValueOnce(
        ok({ writable: true, hasDocument: true, namespaces: [layered] }),
      )
    const { mirror, scope } = derivedScope({ describe: describeCall })
  ...
  it('reports an inherited field as absent from the user layer', async () => {
    const inherited: SettingsNamespaceView = {
      ...view({ preference: 'system' }, 1),
      base: { preference: 'system' },
    }
    const describeCall = vi
      .fn()
      .mockResolvedValueOnce(
        ok({ writable: true, hasDocument: true, namespaces: [inherited] }),
      )
    const { mirror, scope } = derivedScope({ describe: describeCall })
...
describe('SettingsScopeBinder.bind', () => {
  it('shares one mirror read across bound scopes and disposes each with its fiber', async () => {
    const describeCall = vi
      .fn()
      .mockResolvedValue(described({ preference: 'dark' }, 1))
    ...
    new TestRemote(ctx)
    await ctx
      .plugin(SettingsScopeBinder, { mirror, schema: settingsSchema })
      .await()
    ...
    await fiber.await()
    await vi.waitFor(() => {
      expect(theme.getSnapshot()).toMatchObject({
        status: 'ready',
        value: { preference: 'dark' },
      })
      expect(locale.getSnapshot()).toMatchObject({
        status: 'ready',
        value: { preference: 'dark' },
      })
    })
