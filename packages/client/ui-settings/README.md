...
- **Remote settings require deployment opt-in** — a non-loopback browser starts every scope in memory mode and unavailable unless `ctx.connection.canManageHost` is true. A deployment that enables `allowRemoteManagement` gives every client on an allowed authority the same durable settings access as loopback.
