...
- **远程设置需要部署显式开启**：非 loopback 浏览器默认让每个 scope 以 memory 模式和 `unavailable` 状态起步；只有 `ctx.connection.canManageHost` 为 true 时例外。启用 `allowRemoteManagement` 的部署会让获准 authority 上的每个客户端获得与 loopback 相同的持久化设置访问。
