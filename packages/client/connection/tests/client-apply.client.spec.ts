...
afterEach(() => {
  ...
  delete (globalThis as Win).__DSH_CONNECTION__
  sockets.length = 0
...
describe('connection client apply', () => {
  it('mounts ctx.connection with the real client when no ?fixture switch is present', async () => {
    ...
    expect(handle.canManageHost).toBe(true)
  })
  ...
  it('reports non-loopback authority and remote-management capability through the connection handle', async () => {
    (globalThis as Win).location = { hostname: '192.0.2.20', search: '' }
    const remote = await mount()
    expect(remote.isLoopback).toBe(false)
    expect(remote.canManageHost).toBe(false);

    (globalThis as Win).__DSH_CONNECTION__ = { allowRemoteManagement: true }
    expect((await mount()).canManageHost).toBe(true)
  })

  it('rejects a malformed Host-injected connection capability', async () => {
    (globalThis as Win).location = { hostname: '192.0.2.20', search: '' };
    (globalThis as Win).__DSH_CONNECTION__ = { allowRemoteManagement: 'yes' }
    await expect(mount()).rejects.toThrow(
      /allowRemoteManagement must be a boolean/,
    )
  })
  ...
  it('start() hands out one loop, rejects a second consumer, and stop() aborts the streams', async () => {
    ...
    const stopThrowing = handle.hostDescription.subscribe(() => {
  ...
  it('retracts the host description while reconnecting and republishes the next generation', async () => {
        ...
        onStateChange: (state) => {
          if (state === 'reconnecting') {
            reconnectSnapshots.push(
              handle.hostDescription.getSnapshot()?.canOpenPath,
            )
          }
        },
      },
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(reconnectSnapshots).toEqual([undefined])
      })
      await vi.waitFor(() => {
  ...
  it('WebApiClient keeps unary calls and respond on globalThis.fetch', async () => {
    ...
    try {
      ...
      await (handle.api as WebApiClient).host
        .describe({})
        .catch(() => undefined)
      await handle.api
        .respond({
  ...
  it('opens one WebSocket per downlink, parses frames, and aborts both without using fetch', async () => {
    ...
    (globalThis as WebSocketGlobal).WebSocket =
    ...
    const mux = client.events
      .mux({}, muxAbort.signal, () => {
        opened.push('mux')
      })
      [Symbol.asyncIterator]()
    const host = client.events
      .host({}, hostAbort.signal, () => {
        opened.push('host')
      })
      [Symbol.asyncIterator]()
    ...
    expect(await muxFrame).toMatchObject({
      value: {
        rpcId: 'mux-browser',
        payload: { type: 'session/subscribed', lastSeq: 8 },
      },
    })
    expect(await hostFrame).toMatchObject({
      value: {
        rpcId: 'host-browser',
        payload: { type: 'host/remote-event', event: 'commands/change' },
      },
    })
    expect(errors).toHaveBeenCalledTimes(2)
    await vi.waitFor(() => {
      expect(envelopes.flat()).toHaveLength(2)
    })
    ...
    expect(
      sockets.every(socket => socket.readyState === FakeWebSocket.CLOSED),
    ).toBe(true)
    errors.mockRestore()
    fetch.mockRestore()
  })
  ...
  it('maps an HTTPS page origin to a secure WebSocket URL', async () => {
    ...
    await vi.waitFor(() => {
      expect(sockets[0]?.url).toBe('wss://harness.example/api/events.mux')
    })
    abort.abort()
  ...
  it('carries RPC calls without requiring secure-context randomUUID', async () => {
    ...
    try {
      await expect(
        handle.rpc.call('/api', 'goals/create', {
  ...
  it('validates generic RPC transport failures, correlation, and targets', async () => {
    ...
    try {
      await expect(
        handle.rpc.call('/api', 'goals/create', {}, abort.signal),
    ...
    for (const [channel, endpoint] of [
      ...
      await expect(handle.rpc.call(channel, endpoint, {})).rejects.toThrow(
        'invalid RPC target',
      )
    }
  })
  ...
  it('carries Goal Remotes over the same state as the client-only fixture API', async () => {
    ...
    expect(created).toMatchObject({
      ok: true,
      value: { ref: { revision: 1 } },
    })
    if (!created.ok) throw new Error('fixture Goal create failed')
    const ref = (created.value as { ref: { id: string; revision: number } })
      .ref
    const edited = await handle.rpc.call('/api', 'goals/edit', {
      args: {
        agentId: 'fx-alpha',
        ref,
        request: { objective: 'edited fixture remote' },
      },
    })
    expect(edited).toMatchObject({
      ok: true,
      value: { objective: 'edited fixture remote', revision: 2 },
    })
    ...
    expect(paused).toMatchObject({
      ok: true,
      value: { phase: 'paused', activation: 'disarmed', revision: 3 },
    })
    ...
    expect(resumed).toMatchObject({
      ok: true,
      value: { phase: 'active', activation: 'armed', revision: 4 },
    })
    ...
    expect(completed).toMatchObject({
      ok: true,
      value: { phase: 'complete', activation: 'disarmed', revision: 5 },
    })
    await expect(
      handle.rpc.call('/api', 'goals/clear', {
        args: { agentId: 'fx-alpha', ref: { id: ref.id, revision: 5 } },
      }),
    ).resolves.toEqual({ ok: true, value: { id: ref.id, revision: 6 } })
    await expect(handle.rpc.call('/other', 'goals/create', {})).rejects.toThrow(
      /channel.*unavailable/,
    )
    await expect(
      handle.rpc.call('/api', 'unknown/read', {
        args: { agentId: 'fx-alpha' },
      }),
