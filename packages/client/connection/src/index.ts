...
export const Config: z<ConnectionConfig> = z.object({
  ...
  allowRemoteManagement: z.boolean().default(false),
})
 ...
 * privileged methods additionally use an empty trust list by default, which
 * pins them to loopback. `allowRemoteManagement` lets the declared authorities
 * pass that second check and injects the same capability into the boot HTML.
...
export function apply(ctx: Context, config?: ConnectionConfig): void {
  ...
  const allowRemoteManagement = config?.allowRemoteManagement ?? false
  ...
  const fetchHandler = connection.createSharedFetchHandler(API_PATH, {
    ...
    !isTrustedApiRequest(request, allowRemoteManagement ? trustedHosts : [])
      ) {
  ...
  ctx.inject(['apiProxy'], (apiCtx) => {
    ...
    const registerDownlink = (
      ...
      apiCtx.effect(
        () =>
          apiCtx.webServer.registerUpgrade({
            path,
